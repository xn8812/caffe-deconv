...
matcaffe_init_chairs(use_gpu, solver_file, 3);

% load pretrained layer weights
model = caffe('get_weights');
if numsteps == 2,
	model_pretrained = load('examples/chairs/results/chairs_transformer_3conv_inc_mask_fixed_model_iter0030.mat');
else
	model_pretrained = load(sprintf('examples/chairs/results/rnn_t%d_finetuning_model_iter0050.mat', numsteps/2));
end
mapping = [1:7;1:7];
