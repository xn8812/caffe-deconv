...
param = struct('base_lr', 0.001, 'stepsize', 100000, 'snapshot', 100000, 'snapshot_prefix', 'examples/multipie/conv_transformer');
make_solver_file_adagrad(solver_file, model_file, param);
matcaffe_init_multipie(true,solver_file);
batch_size = 100;

% weights = caffe('get_weights');
% pretrain = load([path_to_results 'multipie_lwc_autoencoder_model.mat']);
% for i = 1:4, weights(i).weights = pretrain.weights(i).weights; end
% for i = 7:9, weights(i).weights = pretrain.weights(i-1).weights; end;
% caffe('set_weights', weights);

fid_train = fopen([path_to_results 'multipie_lwc_transformer_80x60x3_inc_train_errors.txt'],'w');
for n = 1:1000
    loss_train_image = 0;
    tic;
    fprintf('LWCTransformer_80x60x3_inc -- processing the %dth iteration.\n',n);
    ...
    fprintf(sprintf('LWCTransformer_80x60x3_inc -- training losses are %f in %f seconds.\n', loss_train_image, toc));
    fprintf(fid_train, '%d %f\n', n, loss_train_image); 
    
    if mod(n,100)==0,
        weights = caffe('get_weights');
        save(sprintf([path_to_results 'multipie_lwc_transformer_80x60x3_model_inc_iter%04d.mat'], n), 'weights');
    end
end    
fclose(fid_train);
