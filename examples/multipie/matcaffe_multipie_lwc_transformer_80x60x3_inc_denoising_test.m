...
use_gpu = true;
solver_file = 'examples/multipie/multipie_lwc_transformer_80x60x3_inc_denoising_solver.prototxt';
...
fid_test = fopen([path_to_results 'multipie_lwc_transformer_80x60x3_inc_denoising_test_errors.txt'],'w');
for n = 10
    loss_test_image = 0;
    tic;
    transformer = load(sprintf([path_to_results 'multipie_lwc_transformer_80x60x3_model_inc_denoising_iter%04d.mat'], n));
...
%    figure(4); montage2(images_recons(:,:,:,1:16),struct('hasChn',1));
    ...
    fprintf(sprintf('LWCTransformer_80x60x3_inc_denoising -- testing losses are %f in %f seconds.\n', loss_test_image, toc));
